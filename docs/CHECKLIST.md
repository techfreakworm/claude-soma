...
- [x] **SOMA_DOMAIN centralized** — single env-knob in `/etc/claude-soma/secrets.env` controls the public dashboard domain. `api/main.py` reads `SOMA_DOMAIN` (default `soma.mayankgupta.in`) to build the CORS origins fallback; `wizard/init.py`'s `render_caddyfile()` reads the same knob. Override with: `SOMA_DOMAIN=yourhost.example.com` in `secrets.env` then restart the API (`sudo systemctl restart claude-soma-api.service`). `HERMES_API_CORS_ORIGINS` still works as a full comma-list override (used by `systemd/claude-soma-api.service` for the live value).
- [x] **FILES_DOMAIN env knob** — `caddy/files.caddyfile.in` is now templated via `__FILES_DOMAIN__`; set `FILES_DOMAIN` in `/etc/claude-soma/secrets.env` (default: `files.mayankgupta.in`) to control the Caddy files site block domain.
