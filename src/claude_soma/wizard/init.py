...
def render_caddyfile(domain: str | None = None, email: str = "") -> str:
    if domain is None:
        domain = os.environ.get("SOMA_DOMAIN", "soma.example.com")
...
def _build_wizard_services(paths: Paths) -> list[tuple[str, str]]:
    ...
    timers = [
        ...
        ("claude-soma-usage-snapshot", "*:0/15"),
    ...
    try:
        defaults = [
            ...
            ("usage-snapshot", "system",
             "every 15 min", "usage-snapshot",
             "Every-15-min Max-credit usage snapshot",
...
def run() -> int:
    ...
    domain = prompt("Public dashboard domain", os.environ.get("SOMA_DOMAIN", "soma.example.com"))
